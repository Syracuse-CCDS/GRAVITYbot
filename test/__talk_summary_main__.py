    ...
    while len(talk_file) < 1:
        ...
        for f in file_search:
            if re.match(f"project-1104-comments_{talk_dat1_end}.csv", f):
                talk_file = f
                print(
                    f'NOTICE: Talk file "{talk_file}" found!\n    Generating date range for summary...\n'
                )
...
def segment_by_time(text_dat, start_date, end_date):
    ...
    talk_dat = text_dat[
        (text_dat["timestamp"] >= start_dt) & (text_dat["timestamp"] <= end_dt)
    ]
...
def main():
    ...
    talk_dat0 = segment_by_time(
        talkload, time_deltas["talk_dat0_start"], time_deltas["talk_dat0_end"]
    )
    talk_dat1 = segment_by_time(
        talkload, time_deltas["talk_dat1_start"], time_deltas["talk_dat1_end"]
    )
    ...
    try:
        ...
        summary_path = (
            config.OUTPUT_FOLDER_PATH / f"ZooniverseTalkSummary_{current_day}.md"
        )
