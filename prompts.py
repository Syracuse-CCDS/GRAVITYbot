...
#####################################################################################################
# Backlog Tasks:
# 1. Consider hashtags in the prompt (Carsten)
# 2. The following response might be valuable for the GravitySpy prompt  
...
# 3. Consider multiple prompts for multiple purposes, and whether they should be automated
#####################################################################################################
def ligo_prompt(talk_dat0, talk_dat1):
    ...
    Using these two sets of data, please provide at least three bullet points to answer the following questions. Each bullet point requires a couple of sentences of response. Under each bullet point, please provide the relevant URLs following that played a part in your response for that bullet following this format: [3685209](https://www.zooniverse.org/projects/zooniverse/gravity-spy/talk/6872/3685209) and format such that they open in a new tab.
    
    I want a list of novel things which occur in "this week's" forum data relative to "last week's."
    
    1. Citizen scientists identify new glitch classifications based on their unique characteristics. Are there new glitches proposed this week? If this week's forum data suggests a new glitch relative to last week, what characterizes the glitch? How is this glitch described? Provide a bullet point for each new glitch with two sentences. Provide the relevant URLs under each bullet point.

    2. Citizen scientists sometimes question characteristics of existing glitches. Are there any questions regarding the glitch classes according to the citizen scientists? Are these questions about limitations of existing classifications or are they merely requiring clarifications about existing glitches? If the former, then this is more relevant. If the latter, then it is less of a priority. Provide two sentences describing each question raised.  Provide the relevant URLs under each bullet point.
    ...
    4. Glitches are often related to ecological factors near the sensors or errors in the sensors themselves. Citizen scientists often attempt to analytically explain glitch occurrences with hypothetical origins of glitches, suggesting gained knowledge of the mechanisms of glitch occurrences. If this week has more specific hypotheses about where glitch origins, what are these hypotheses and the reasons provided? Provide at two or three sentences describing each hypothesis. Provide the relevant URLs under each bullet point.

    5. Citizen scientists often question if issues are related to particular sensors or channels. Are there any emerging attention or questions surrounding particular glitches' connections to sensors or channels this week relative to last week? If so, what specifically do they describe? Provide at least two sentences for these questions or thoughts. Provide the relevant URLs under each bullet point.
    """

    # Optional: provide a system prompt which tells the bot the context it is responding to.
    sys_prompt = f"""
    You are a technical interpreter who translates citizen science forum conversations for physicists and engineers. You are to describe particulars of where this ambiguity appears to come from.  Format all relevant hyperlinks without hashtags following this format:  [3685209](https://www.zooniverse.org/projects/zooniverse/gravity-spy/talk/6872/3685209).
    """
...
def alog_prompt(alog_dat0, alog_dat1):
    # Required: provide a user prompt variable which takes the text data
    user_prompt = f"""
    The data involve discussions surrounding LIGO laboratory equipment. The data originally were in a dataframe of two columns. The first column was the "comment" text and the second was the "URL" affiliated with the comment. After each comment has been formatted such that it is followed by its URL. Format all URLs without hashtags following this format: [75875](+tab+https://alog.ligo-la.caltech.edu/aLOG/index.php?callRep=75875).
